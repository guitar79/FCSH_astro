...
\textrm{Figure}. \ref{FSQ}는 FSQ-106ED의 제조사인 타카하시 사의 홈페이지에서 제공하는 경통의 대략적인 정보이다. FSQ-106ED 경통은 필드 플레트너가 내장된 경통으로 천체사진을 촬영하는 아마추어 천문가들이 애용하는 소형 천체망원경 중 고급형 모델로 분류된다. 본 연구에서  연구에 개발된 구동부는 이 경통에 맞게 제작하였다. 경통의 사양을 참조하여  후드의 안쪽 지름은 $180 \textrm{ mm}$이 되도록 제작하였으며, 바흐티노프 마스크 또한 망원경에 맞추어 구경 (aperture) 값을 $106 \textrm{ mm}$, 초점거리 (focal length) 값을 $530 \textrm{ mm}$의 값으로 출력하였다.


\subsubsection{후드 제작}


경통의 후드는 \textrm{Figure}. \ref{fig:cover}a와 같이 오토데스크 사의 Fusion360 (https://www.autodesk.co.kr/products/fusion-360/students-teachers-educators)을 이용하여 설계한 후, 3D 프린터로 출력하여 제작하였다. 경통의 후드는 천체망원경 별로 경통의 지름과 같은 특성이 다르고, 천체망원경 위에 고정시킬 수 있을 만큼 견고해야한다. 때문에 경통에 씌울 수 있도록 지름을 계산하여 팔각형 모양으로 감싸는 형태로 제작하였으며, 3D 프린터에서 출력할 수 있는 크기 제한이 있고 동시에 천체망원경에 부착시킬 때 편리하게 할 수 있도록 총 네 조각으로 나누어 조립하는 방식을 택하였다. 네개 조각의 렌치 볼트와 너트로 조립하여 완성할 수 있다.
...
\begin{figure}[h]
	\begin{center}
		...
		\node[anchor=south west,inner sep=0] at (0,0) 
		{
			\includegraphics[height=4.2cm]{thermicline1}
			\includegraphics[height=4.2cm]{thermicline3} 
		};
		\draw (0.3, 3.7) node {(a)};
		\draw (6.4, 3.7) node {(b)};
		\end{tikzpicture}
...
본 연구에서는 이러한 단점들을 보완하여 EEPROM을 적용시키고 열선을 사용 가능하도록 하는 등의 편의 기능들을 개선하였으며, 이를 사용하여 소형 천체망원경의 자동화를 하고자 한다.
