...
Fig. \ref{fig:focuserchooser}\은 ASCOM에서 지원하는 focuser chooser 화면을 띄웠을 때 나타나게 되는 창이다. 창 자체는 ASCOM Diagnostic을 통해서도 실행할 수 있지만, ASCOM Diagnostic은 어디까지나 connection test 프로그램이기 때문에 오직 ASCOM을 통해 연결이 되는지만 확인이 (본 연구에서는 mainWindow.cs가 보이지 않는 현상이 있었다.) 따라서 실제로 모터가 움직이는 것까지 확인하기 위해서는 MaximDL이나 FocuserMax와 같은 다른 프로그램을 활용해야한다.
