...
%\begin{figure}[h]
%	\subfloat{\includegraphics[width = 5.5cm]{arduino_nano-1}} 
%	\subfloat{\includegraphics[width = 3.5cm]{DRV8825-1}}
%	\subfloat{\includegraphics[width = 3.5cm]{DHT22-1}}
%	\subfloat{\includegraphics[width = 3.5cm]{OLED-1}} 
...
\begin{figure}[h]
	...
	\draw (0.5, 3.0) node {(a)};
	\draw (6.0, 3.0) node {(b)};
	\draw (10.0, 3.0) node {(c)};
	\draw (12.8, 3.0) node {(d)};
	\end{tikzpicture}
	\end{center}
...
\begin{figure}[h]
	\begin{center}
		...
		\node[anchor=south west,inner sep=0] at (0,0) 
		{
			{\includegraphics[width=1.0\linewidth]{prototype}} 
		};
		\draw (0.3, 7.8) node {(a)};
		\draw (8.3, 7.8) node {(b)};
		\end{tikzpicture}
...
Fig. \ref{fig:circuit_maker}\는 Circuit maker를 실행한 화면을 나타낸 것이다. Circuit maker 사용법은 유투브 강좌를 통해 익혔다. Circuit maker 사용법을 익히는데 숙지하는 데 오랜 시간이 걸렸기 때문에, 그 사이에 진행된 연구에서는 Fig. \ref{fig:prototype}의 프로토 타입 하드웨어를 이용하여 펌웨어 개발을 수행하였다. 
...
%\begin{figure}[h]
%	\begin{center}
%		\includegraphics[width=0.5\linewidth]{circuit_maker2}
%		\includegraphics[width=0.5\linewidth]{circuit_maker}
