...
namespace ASCOM.GS_touch
{
    public partial class MainWindow : Form
    {
        ...
        {
            focuser.Action("X", numericUpDown_Acceleration.Value.ToString());
        }
        //일단 X에 PWM 할당
        ...
        private FCSH fCSH;
        private void button_maskControl_Click(object sender, EventArgs e)
        {
            fCSH = new FCSH(this);
            fCSH.Show();
        }
        public void applyFCSH(int type)
        {
            if (type==1) focuser.Action("N", "");
            else if (type == 2) focuser.Action("O", "");
            else if (type == 3) focuser.Action("P", "");
            else if (type == 4) focuser.Action("Q", "");
            else if (type == 5) focuser.Action("R", "");
            else if (type == 6) focuser.Action("S", "");
            return;
        }
        public int askFCSH()
        {
