...
{
#ifdef INOUTLEDS
  (!GSfocuser.ReverseDirection) ? digitalWrite( gledOUT, 1) : digitalWrite( bledIN, 1);
#endif
  (!GSfocuser.ReverseDirection) ? digitalWrite(myDir, LOW ) : digitalWrite(myDir, HIGH );
  digitalWrite(myStep, 1 );
  delayMicroseconds(stepontime);
  digitalWrite(myStep, 0 );
#ifdef INOUTLEDS
  (!GSfocuser.ReverseDirection) ? digitalWrite( gledOUT, 0 ) : digitalWrite( bledIN, 0);
#endif
}
...
{
#ifdef INOUTLEDS
  (!GSfocuser.ReverseDirection) ? digitalWrite( bledIN, 1) : digitalWrite( gledOUT, 1);
#endif
  (!GSfocuser.ReverseDirection) ? digitalWrite(myDir, HIGH ) : digitalWrite(myDir, LOW );
  digitalWrite(myStep, 1 );
  delayMicroseconds(stepontime);
  digitalWrite(myStep, 0 );
#ifdef INOUTLEDS
  (!GSfocuser.ReverseDirection) ? digitalWrite( bledIN, 0 ) : digitalWrite( gledOUT, 0);
#endif
}
...
void updatemotorSpeedDelay()
{
  ...
  //Serial.print("stepmode = "); Serial.println(GSfocuser.stepmode);
...
{
  GSfocuser.validdata = 99;
  GSfocuser.fposition = 5000L;
  GSfocuser.maxstep = 10000L;
  GSfocuser.stepmode = 1;
  GSfocuser.ReverseDirection = 0;
  GSfocuser.coilPwr = 1;
  GSfocuser.tempmode = 1; // celsius
  GSfocuser.updatedisplayintervalNotMoving = 2500L;
  GSfocuser.ds18b20resolution = TEMP_PRECISION;
  GSfocuser.stepsizeenabled = 0;
  GSfocuser.stepsize = 10;
  GSfocuser.lcdupdateonmove = 0;
  // now write the data to EEPROM
  EEPROM_writeAnything(currentaddr, GSfocuser);    // update values in EEPROM
}
...
void processCommand()
{
  ...
  switch ( cmdval )
  {
    ...
    case 21319: // get stepmode
      sprintf(tempString, "%02X", GSfocuser.stepmode);
      replystr = String(tempString) + "#";
      sendresponsestr(replystr);
      break;
    case 20295: // :GO# get the coilPwr setting
      replystr = String(GSfocuser.coilPwr) + "#";
      sendresponsestr(replystr);
      break;
    case 21063: // :GR# get the Reverse Direction setting
      replystr = String(GSfocuser.ReverseDirection) + "#";
      sendresponsestr(replystr);
      break;
    ...
    case 22599: // :GXxxxxx#          get the time that an LCD screen is displayed for (in milliseconds, eg 2500 = 2.5seconds
      sprintf(tempString, "%04X", (unsigned int) GSfocuser.updatedisplayintervalNotMoving);
      replystr = String(tempString) + "#";
      sendresponsestr(replystr);
      break;
    case 18256: // :PG    Get temperature precision (9-12)
      ...
      if ( tprobe1 == 1 )
      {
        GSfocuser.ds18b20resolution = (byte) sensor1.getResolution(tpAddress);
        tmp = GSfocuser.ds18b20resolution;
    ...
    case 20051: // :SNxxxx# set new target position SNXXXX - this is a move command
      ...
      if ( pos < 0 )
        pos = 0;
      else if ( pos > GSfocuser.maxstep)
        pos = GSfocuser.maxstep;
      targetPosition = pos;
      gotonewposition = 0;
      isMoving = 0;
      break;
    ...
    case 20307: // :SOxxxx# set the coilPwr setting
      GSfocuser.coilPwr = (byte) decstr2int(param) & 0x01;
      writenow = 1;
      previousMillis = millis();
      break;
    case 21075: // :SRxx# set the Reverse Direction setting
      GSfocuser.ReverseDirection = (byte) decstr2int(param) & 0x01;
      writenow = 1;
      previousMillis = millis();
      break;
    case 19780: // :DMx# set displaystate C or F
      GSfocuser.tempmode = (byte) decstr2int(param) & 0x01;
      writenow = 1;
      previousMillis = millis();
      break;
    ...
    case 22605: // :MX#          None        Save settings to EEPROM
      ...
      GSfocuser.validdata = 99;
      GSfocuser.fposition = currentPosition;
      GSfocuser.maxstep = maxSteps;
      EEPROM_writeAnything(currentaddr, GSfocuser);    // update values in EEPROM
      writenow = 0;
      break;
    ...
    case 22611: // :SXxxxx# None    Set updatedisplayNotMoving (length of time an LCD page is displayed for in milliseconds
      ...
      if ( pos > 4000L )
        pos = 4000L;
      GSfocuser.updatedisplayintervalNotMoving = pos;
      writenow = 1;             // updating of EEPROM ON
      previousMillis = millis();   // start time interval
      break;
    ...
    case 21328: // :PS    Set temperature precision (9-12 = 0.5, 0.25, 0.125, 0.0625)
      ...
      if ( tprobe1 == 1 )
      {
        ...
        GSfocuser.ds18b20resolution = (byte) tmp;        // save setting in EEPROM
        writenow = 1;             // updating of EEPROM ON
        previousMillis = millis();   // start time interval
      }
      else
    ...
    case 19792: // :PMxx#    None    set update of position on lcd when moving (00=disable, 01=enable)
      GSfocuser.lcdupdateonmove = (byte) decstr2int(param) & 0x01;
      writenow = 1;             // updating of EEPROM ON
      previousMillis = millis();   // start time interval
      break;
    case 23120: // :PZxx#   None      Set the return of user specified stepsize to be OFF - default (0) or ON (1)
      GSfocuser.stepsizeenabled = (byte) decstr2int(param) & 0x01;
      writenow = 1;             // updating of EEPROM ON
      previousMillis = millis();   // start time interval
      break;
    case 20560: // :PPxxxx#  None     Set the step size value - double type, eg 2.1
      ...
      GSfocuser.stepsize = (double) tmpstr.toFloat();
      if ( GSfocuser.stepsize < 0 )
      {
        GSfocuser.stepsize = 0;
      }
      writenow = 1;             // updating of EEPROM ON
      previousMillis = millis();   // start time interval
      break;
    case 23128: // :XZ# troubleshooting only - reset focuser defaults
      ...
      currentPosition = GSfocuser.fposition;
      targetPosition = GSfocuser.fposition;
      maxSteps = GSfocuser.maxstep;
      break;
  }
}
...
void displaylcd()
{
  ...
  // stepmode setting
  if ( GSfocuser.stepmode == 1 )
    myoled.print("Step Mode   =Full");
  else if ( GSfocuser.stepmode == 2 )
    myoled.print("Step Mode   =Half");
  else if ( GSfocuser.stepmode == 4 )
    myoled.print("Step Mode   =1/4");
  else if ( GSfocuser.stepmode == 8 )
    myoled.print("Step Mode   =1/8");
  else if ( GSfocuser.stepmode == 16 )
    myoled.print("Step Mode  =1/16");
  else if ( GSfocuser.stepmode == 32 )
  ...
  ltoa(GSfocuser.maxstep, tempString, 10);
