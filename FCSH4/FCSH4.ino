// GSfocuserPro DRV8825-HW203-OLED-BT
// GSfocuserPro2M PCB and Case - Fully Assembled and Ready to go
...
// Permission is NOT granted to any person to redistribute, market, manufacture or sell for commercial gain the GSfocuser
...
// https://www.dropbox.com/s/vgr8glsjfi5omom/OrderForm-GSfocuserPro2M.pdf?dl=0
//
// Order PCB
// https://aisler.net/brown_rb/my-project-repository/GSfocuserpro2mdrv8825
...
// Compatible with GSfocuser ASCOM driver
// Compatible with GSfocuser Application
...
// heavily modified by RBB to work with GSfocuser
// ----------------------------------------------------------------------------------------------------------
...
void setup()
{
  ...
  byte datasize;     // will hold size of the struct GSfocuser - 6 bytes
  ...
  if ( found == 1 )
  {
    EEPROM_readAnything( currentaddr, GSfocuser );
    GSfocuser.validdata = 0;
    EEPROM_writeAnything(currentaddr, GSfocuser);
  ...
  currentPosition = GSfocuser.fposition;  // Set focuser defaults from saved values in EEPROM.
  ...
  if ( tprobe1 == 1 )
  {
    sensor1.setResolution( tpAddress, GSfocuser.ds18b20resolution );  // set probe resolution
    requesttemp();
    delay(600 / (1 << (12 - GSfocuser.ds18b20resolution)));           // should enough time to wait
    readtemp();
  }
  requesttempflag = 0;
#endif
  ...
  motorspeedchange = 0;
  setstepmode(GSfocuser.stepmode);
  motorSpeed = FAST;
  savedmotorSpeed = FAST;
  updatemotorSpeedDelay();

  if ( GSfocuser.updatedisplayintervalNotMoving < 2000 )
    GSfocuser.updatedisplayintervalNotMoving = 2000;
  if ( GSfocuser.updatedisplayintervalNotMoving > 4000 )
    GSfocuser.updatedisplayintervalNotMoving = 4000;
...
void loop()
{
  ...
  if ((targetPosition != currentPosition) && (gotonewposition == 1))
  {
    ...
    if ( GSfocuser.lcdupdateonmove == 1 )
    {
      updatecount++;
  ...
  else
  {
    ...
    if ( ((currentMillis - olddisplaytimestampNotMoving) > GSfocuser.updatedisplayintervalNotMoving) || (currentMillis < olddisplaytimestampNotMoving) )
    {
