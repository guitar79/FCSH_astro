// myfocuserPro DRV8825-HW203-ROTARY ENCODER
...
// To enable Bluetooth on any controller, uncomment BLUETOOTH - cannot use with Rotary Encoder
// To enable the rotary encoder, uncomment ROTARYENCODER - cannot use with Bluetooth or Pushbuttons

// To enable the rotary encoder, uncomment the next line - do not use with bluetooth or pushbuttons
...
// To enable the Push Buttons for manual focusing, uncomment the next line - do not use with rotary encoder
...
#ifdef ROTARYENCODER
#ifdef BLUETOOTH
#halt  // THIS IS INVALID - YOU CANNOT HAVE BOTH THE BLUETOOTH AND ROTARYENCODER OPTIONS ENABLED
// PLEASE DISABLE ONE
#endif
#endif
...
#ifdef ROTARYENCODER
#ifdef PUSHBUTTONS
#halt   // THIS IS INVALID - YOU CANNOT HAVE BOTH THE PUSHBUTTONS AND ROTARYENCODER OPTIONS ENABLED
// PLEASE DISABLE ONE
#endif
#endif
...
#define myM1            6
#define myM0            7   // M0/M1/M2 sets stepping mode 000=F, 100=1/2, 010=1/4, 110=1/8, 001=1/16, 101=1/16, 110=1/16 etc
...
const String programName = "myFP-DRV8825HW203RE";
...
void loop()
{
  ...
  if ( isMoving == 0 )
  {
    ...
    if ( lp != 0 ) {
      ...
      targetPosition = targetPosition + (myrotaryencoder.getposition() * encswval);
      // bounds check the move
      if ( targetPosition < 1 )
        targetPosition = 0;
      if ( targetPosition >= maxSteps )
        targetPosition = maxSteps;
      // Serial.println(targetPosition); // debug message only, do not uncomment
      myrotaryencoder.setposition(0);    // reset to avoid counting more than once
      gotonewposition = 1;           // needed to GO once focus position has changed
    }
  }
  ...
  if ( RESWVal == 0 )
  {
    ...
    // Serial.print("Pressed");   // debug message only, do not uncomment

    long encswstart = millis();    // start time of encoder sw press
    // wait for PB to be released
    while ( !digitalRead(encoderSWPin))
      ;
    RESWdb.update();            // clear the encoder switch value
    encswpress = millis();      // end time of encoder sw press
    analogWrite(Buzzer, 0);     // turn off buzzer
    long encswtime = encswpress - encswstart;  // how long enc sw was pressed
    // Serial.print("Released");   // debug message only, do not uncomment

    if ( encswtime < 1000 )
    {
      // less than 1s then increment = 1
      beep(1);                     // provide audible feedback
      motorSpeed = 0;
      updatemotorSpeedDelay();
      encswval = 1;
    }
    else if (encswtime < 2000 )
    {
      // 1.0-1.9s, then increment by 10
      beep(2);
      motorSpeed = 1;
      updatemotorSpeedDelay();
      encswval = 10;
    }
    else if (encswtime < 3000 )
    {
      // 2.0-2.9s, increment by 100
      beep(3);
      motorSpeed = 2;
      updatemotorSpeedDelay();
      encswval = 100;
