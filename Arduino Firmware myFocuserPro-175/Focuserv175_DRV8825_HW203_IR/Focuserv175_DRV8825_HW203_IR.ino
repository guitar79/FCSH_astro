...
// To enable Infra Red Controller, uncomment INFRARED (recommended that you disable PUSHBUTTONS)
...
const String programName = "myFP2-DRV8825HW203IR";
...
void loop()
{
  ...
  if ( isMoving == 0 )
  {
    if (irrecv.decode(&results))
    {
      ...
      // must be type static so that it is remembered whilst arduino is running
      // it holds the last recieved code, so that if the user holds down the key and a repeat code
      // is generated, then the data code can be recalled
      ...
      if ( results.value == 4294967295 ) { // repeat last code
      ...
      targetPosition = targetPosition + adjpos;
      gotonewposition = 1;
